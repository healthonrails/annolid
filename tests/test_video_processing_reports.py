...
def test_save_processing_summary_writes_per_video_override_section(
    tmp_path: Path, monkeypatch
) -> None:
    output_dir = tmp_path / "output"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    reports_mod.save_processing_summary(
        ...
        video_paths=[str(output_dir / "clip_fix.mp4")],
        input_mode="folder",
        input_source=str(tmp_path / "input"),
        output_folder=str(output_dir),
        scale_factor=0.5,
        fps=15.0,
        apply_denoise=True,
    ...
    md_text = (output_dir / "clip_fix.md").read_text(encoding="utf-8")
    assert "Per-video review selection: yes" in md_text
    assert "Effective Scale Factor: 0.25" in md_text

...
def _make_shifted_frames(shift_x: float = 1.0, shift_y: float = 0.0, size: int = 64):
    ...
    center_curr = (int(center_prev[0] + shift_x),
                   int(center_prev[1] + shift_y))
...
@pytest.mark.skipif(RAFTOpticalFlow is None, reason="torchvision optical_flow not available")
def test_raft_flow_detects_shift():
    ...
    prev_t = torch.from_numpy(prev).permute(
        2, 0, 1).unsqueeze(0).float().to(device)
    curr_t = torch.from_numpy(curr).permute(
        2, 0, 1).unsqueeze(0).float().to(device)

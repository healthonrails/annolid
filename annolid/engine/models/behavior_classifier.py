...
class BehaviorClassifierPlugin(ModelPluginBase):
    ...
    def add_train_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--feature-backbone",
                            choices=BACKBONE_CHOICES, default="dinov3")
        parser.add_argument("--dinov3-model-name", type=str,
    ...
    def train(self, args: argparse.Namespace) -> int:
        ...
        val_loader = DataLoader(val_dataset, batch_size=int(
        ...
        tb_dir_raw = str(
            getattr(args, "tensorboard_log_dir", "") or "").strip()
        if tb_dir_raw:
    ...
    def add_predict_args(self, parser: argparse.ArgumentParser) -> None:
        parser.add_argument("--video-folder", required=True,
                            help="Folder of videos for inference")
        parser.add_argument("--checkpoint-path", required=True,
                            help="Path to trained weights (.pth)")
        parser.add_argument("--batch-size", type=int, default=1)
        parser.add_argument("--feature-backbone",
                            choices=BACKBONE_CHOICES, default="clip")
        parser.add_argument("--dinov3-model-name", type=str,
                            default=DEFAULT_DINOV3_MODEL)
        parser.add_argument("--feature-dim", type=int, default=None)
        parser.add_argument("--transformer-dim", type=int, default=768)

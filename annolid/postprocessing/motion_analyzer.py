...
def analyze_motion_index(csv_path: str, output_dir: str = "."):
    ...
    df_filtered = df[df["motion_index"] >= 0].dropna(subset=["motion_index"]).copy()
    ...
    print(f"✅ Data Loaded: Analyzing {total_frames} frames with valid motion data.")

    df_filtered["plot_motion_index"] = df_filtered["motion_index"].replace(0, 1e-9)
    ...
    resting_frames = df_filtered[df_filtered["motion_index"] < resting_threshold]
    ...
    print(f"High Activity Threshold (95th percentile): {high_activity_threshold:.4f}")
    ...
    df_filtered["behavioral_state"] = df_filtered["motion_index"].apply(classify_motion)

    state_distribution = (
        df_filtered["behavioral_state"]
        .value_counts(normalize=True)
        .mul(100)
        .reindex(["Resting", "Moderate Activity", "High Activity"])
    )
    ...
    outlier_df = df_filtered[df_filtered["motion_index"] > outlier_threshold].copy()

    recommendations = pd.DataFrame()
    if not outlier_df.empty:
        # Identify contiguous blocks of outlier frames to define distinct events
        outlier_df["event_block"] = (outlier_df["frame_number"].diff() > 1).cumsum()

        # Aggregate each event block to find start, end, peak frame, and peak motion
        event_summary = (
            outlier_df.groupby("event_block")
            .agg(
        ...
        peak_frames = outlier_df.loc[
            outlier_df.groupby("event_block")["motion_index"].idxmax()
        ]
        event_summary = event_summary.merge(
            peak_frames[["event_block", "frame_number"]], on="event_block"
        )
        event_summary.rename(columns={"frame_number": "peak_frame"}, inplace=True)

        # Sort events by intensity (most extreme first)
        recommendations = event_summary.sort_values(
            "peak_motion", ascending=False
        ).head(10)
    ...
    fig.suptitle(
        f"Behavioral Activity Dashboard: {os.path.basename(csv_path)}",
        fontsize=24,
        y=0.97,
    )
    ...
    state_palette = {
        "Resting": "#2ca02c",
        "Moderate Activity": "#ff7f0e",
        "High Activity": "#d62728",
    }
    ...
    sns.scatterplot(
        ...
        alpha=0.8,
        linewidth=0,
        hue_order=["Resting", "Moderate Activity", "High Activity"],
        legend="full",
    )
    ...
    if not recommendations.empty:
        for idx, row in recommendations.head(5).iterrows():
            ...
            ax1.annotate(
                ...
                fontsize=12,
                bbox=dict(
                    boxstyle="round,pad=0.3", fc="yellow", ec="black", lw=1, alpha=0.8
                ),
            )
    ...
    sns.barplot(
        ax=ax2,
        x=state_distribution.index,
        y=state_distribution.values,
        palette=state_palette,
        order=["Resting", "Moderate Activity", "High Activity"],
    )
    ...
    output_filename = os.path.join(
        output_dir, os.path.splitext(os.path.basename(csv_path))[0] + "_dashboard.png"
    )

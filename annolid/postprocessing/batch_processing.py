...
def get_videos(videos_dir="/data/videos/", video_file_pattern="*/*.mp4"):
    videos = glob.glob(os.path.join(videos_dir, video_file_pattern))
    assert len(videos) > 0
    return videos


def get_tracking_csvs(csv_dir="/data/csvs/", csv_file_pattern="*/*.csv"):
    tracking_res = {}
    csv_files = glob.glob(os.path.join(csv_dir, csv_file_pattern))
    assert len(csv_files) > 0
    for cs in csv_files:
        video_name_no_ext = (
            os.path.basename(cs).split("dataset_")[1].split("_mask_r")[0]
        )
...
def get_segmentor(
    model_path="model_final.path", dataset_dir="/data/coco/dataset", score_threshold=0.1
):
...
def fill_missing_predictions(
    tracking_csv_file, excluded_instances=None, moving_window=5
):
    """Fill missing predictions.
    Use the last present location as the hidden animals' predictions.
    ...
    print("Fill instances with names in the list:", all_instance_names)
    missing_predictions = []
...
def parse_arges(args=sys.argv[1:]):
    ...
    parser.add_argument(
        "--video_folder", help="folder contains all the video files", required=True
    )
    parser.add_argument(
        "--video_pattern",
    ...
    parser.add_argument(
        "--csv_folder", help="folder with tracking csv files", default="/data/"
    )
    parser.add_argument(
        "--csv_pattern", help="pattern for matching tracking csv files", default="*.csv"
    )
    parser.add_argument(
        "--model_path",
    ...
    parser.add_argument(
        "--motion_threshold", help="motion threshold between 0 to 1", default=0.01
    )
    parser.add_argument("--score_threshold", help="class score threshold", default=0.1)
...
def main():
    ...
    if os.path.exists(args.model_path) and os.path.exists(args.dataset_dir):
        segmentor = get_segmentor(
            args.model_path, args.dataset_dir, args.score_threshold
        )
    for v in videos:
        if segmentor:

...
class FreezingAnalyzer:
    ...
    def get_video_writer(
        self, video_file, target_fps, width, height, video_suffix="results"
    ):
        video_name = f"{os.path.splitext(video_file)[0]}_{video_suffix}.mp4"
        video_writer = cv2.VideoWriter(
            video_name, cv2.VideoWriter_fourcc(*"mp4v"), target_fps, (width, height)
        )
        return video_writer
    ...
    def run(
        ...
        video_writer = self.get_video_writer(
            self.video_file, frames_per_second, width, height
        )
            ...
            if ret:
                ...
                for index, _row in df_prvs_cur.iterrows():
                    if _row.segmentation_y:
                        ...
                        bgr = draw.draw_binary_masks(bgr, [_mask], [_row.instance_name])
                    ...
                    if (
                        _row.mask_iou >= self.iou_threshold
                        and mask_motion < self.motion_threshold
                    ):

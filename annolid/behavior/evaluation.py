...
def _load_data(predicted_segments_path, manual_labels_path):
    ...
    try:
        ...
        raise pd.errors.ParserError("Error parsing CSV file(s). Check the format.")
...
def _create_time_intervals(start_time, end_time, interval_duration):
    ...
    return pd.DataFrame(
        {
            "Recording time": np.arange(
                start_time, end_time + interval_duration, interval_duration
            )
        }
    )
...
def _align_behaviors_to_intervals(df, intervals_df, tolerance):
    ...
    for _, row in df.iterrows():
        ...
        if abs(closest_time - row["Recording time"]) <= tolerance:
            merged_df.loc[merged_df["Recording time"] == closest_time, "Behavior"] = (
                row["Behavior"]
            )
    return merged_df
...
def evaluate_behavior_predictions(
    predicted_segments_path, manual_labels_path, interval_duration=3
):
    """Evaluates behavior predictions against manual labels using a fixed interval approach."""

    predicted_df, manual_df = _load_data(predicted_segments_path, manual_labels_path)

    start_time = min(
        predicted_df["Recording time"].min(), manual_df["Recording time"].min()
    )
    end_time = max(
        predicted_df["Recording time"].max(), manual_df["Recording time"].max()
    )

    intervals_df = _create_time_intervals(start_time, end_time, interval_duration)

    tolerance = interval_duration / 2
    merged_manual = _align_behaviors_to_intervals(manual_df, intervals_df, tolerance)
    merged_predicted = _align_behaviors_to_intervals(
        predicted_df, intervals_df, tolerance
    ).rename(columns={"Behavior": "Behavior_predicted"})

    merged_df = pd.merge(
        merged_manual, merged_predicted, on="Recording time", how="outer"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(results)
        plot_confusion_matrix(merged_df["Behavior"], merged_df["Behavior_predicted"])

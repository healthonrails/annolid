...
def parse_args():
    arg_builder = argparse.ArgumentParser(description="Multiple Animal Tracking")
    arg_builder.add_argument(
        "-v", "--video", type=str, default=None, help="path to a input video file"
    )
    arg_builder.add_argument(
        "--extract_frames",
        type=int,
    ...
    arg_builder.add_argument(
        "--vis", type=bool, default=False, help="Visualize the labeled images"
    )
    arg_builder.add_argument(
        "--tracks2glitter", type=str, default=None, help="tracking results csv file."
    )
    arg_builder.add_argument(
        "--seg_or_detect",
        type=str,
...
def main():
    ...
    if args["segmentation"] == "threshold":
        ir = InRange()
        ir.run(args["video"], args["min_area"], args["max_area"])

    if args["track"] is not None:
        track(args["video"], args["track"], args["weights"])
    if args["coco2yolo"] is not None:
        names = coco2yolo.create_dataset(
            args["coco2yolo"],
            results_dir=args["to"],
    ...
    if args["labelme2yolo"] is not None:
        ...
        converter.convert(val_size=args["val_size"], test_size=args["test_size"])
        print("Done")
    ...
    if args["tracks2glitter"] is not None:
        assert os.path.isfile(args["tracks2glitter"]), (
        ...
        tracks2nix(args["video"], args["tracks2glitter"], dest_file)

    if (args["keypoints2labelme"] is not None) and (args["keypoints"] is not None):
        from annolid.annotation import keypoints

        keypoints.to_labelme(
            args["keypoints2labelme"], args["keypoints"], args["multiple_animals"]
        )

from qtpy.QtWidgets import (QDialog, QVBoxLayout, QFormLayout, QLabel, QLineEdit,
                            QPushButton, QSpinBox, QRadioButton, QButtonGroup,
                            QTableView, QHeaderView, QHBoxLayout, QMessageBox,
                            QAbstractItemView, QDialogButtonBox, QGroupBox, QWidget)
from qtpy.QtCore import Slot, Signal, Qt, QTimer
...
class SegmentEditorDialog(QDialog):
    ...
    tracking_initiated = Signal(
        TrackingWorker, Path)  # worker_instance, video_path_processed
    ...
    def __init__(self,
        ...
        if not active_video_path or active_video_fps <= 0 or active_video_total_frames <= 0:
            QTimer.singleShot(0, self.reject)
            logger.error(
                "SegmentEditorDialog: Initialized with invalid video context.")
            return
        ...
        self.update_live_annolid_frame_info(
            current_annolid_frame)  # Initial display of live frame
    ...
    def _setup_ui(self):
        ...
        info_layout.addRow("Annolid Frame (Live):",
                           self.live_annolid_frame_label)
        ...
        self.annotation_frame_input.setRange(
            0, self._active_video_total_frames - 1)
        ...
        self.end_time_radio = QRadioButton(
            "End Time (HH:MM:SS):")  # Will add validator later
        ...
        self.end_condition_group.buttonToggled.connect(
            self._toggle_end_condition_inputs_enabled_state)  # Use buttonToggled
        ...
        self.segments_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch)  # Stretch last section later if needed
        ...
        self.track_all_segments_button = QPushButton(
            "Track All Defined Segments")
    ...
    def _initiate_tracking_from_dialog(self):
        ...
        valid_segments_to_track = [
            s for s in all_segments if s.is_annotation_valid()]
        invalid_count = len(all_segments) - len(valid_segments_to_track)

        if invalid_count > 0:
            msg = (f"{invalid_count} segment(s) have missing/invalid annotation JSONs and will be skipped. "
                   "Proceed with valid segments?")
            reply = QMessageBox.warning(
                self, "Invalid Segments", msg, QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.No:
                return

        if not valid_segments_to_track:
            QMessageBox.warning(self, "No Valid Segments",
        ...
        if hasattr(annolid_main_window, 'is_tracking_busy') and annolid_main_window.is_tracking_busy():
            QMessageBox.warning(self, "Tracking Busy",
        ...
        job = VideoProcessingJob(
            ...
            video_specific_config=self.annolid_config.get(
                'advanced_tracking_params', {})  # From Annolid
        )
        jobs_list = [job]
        ...
        try:
            ...
            self.track_all_segments_button.setEnabled(
                False)  # Disable while this worker runs
            ...
            logger.error(
                f"Failed to create or start TrackingWorker from dialog: {e}", exc_info=True)
            QMessageBox.critical(self, "Worker Error",
                                 f"Could not start tracking worker: {e}")
            self.internal_status_label.setText(
    ...
    def _on_dialog_worker_finished(self, message: str):
        ...
        if sender_worker:
            try:
                sender_worker.finished.disconnect(
                    self._on_dialog_worker_finished)
    ...
    def update_live_annolid_frame_info(self, frame_number: int):
        ...
        time_str = TrackingSegment._format_seconds(
            frame_number / self._active_video_fps) if self._active_video_fps > 0 else "N/A"
    ...
    def _on_use_current_annolid_frame(self):
        if self._current_annolid_frame_dynamic is not None:
            self.annotation_frame_input.setValue(
                self._current_annolid_frame_dynamic)
    ...
    def _reset_form_to_defaults(self):
        ...
        self.annotation_frame_input.setValue(
            self._current_annolid_frame_dynamic or 0)
        self._on_annotation_frame_input_changed(
            self.annotation_frame_input.value())  # Validate

        self.duration_radio.setChecked(True)
        self.duration_spinbox.setValue(60)  # Default 60 seconds
        self.end_time_input.setText(TrackingSegment._format_seconds(
            (self.annotation_frame_input.value() / self._active_video_fps) +
            60 if self._active_video_fps > 0 else 60
        ))
        self.end_frame_spinbox.setValue(min(
            self.annotation_frame_input.value() + int(60 *
    ...
    def _on_add_or_update_segment(self):
        ...
        if not self._validate_annotation_json(annotated_frame):
            QMessageBox.warning(self, "Missing Annotation",
        ...
        try:
            if self.duration_radio.isChecked():
                ...
                num_frames_for_duration = int(
                    duration_sec * self._active_video_fps)
                segment_end_frame = min(
                    segment_start_frame + num_frames_for_duration - 1, self._active_video_total_frames - 1)
                ...
                segment_end_frame = min(int(
                    total_seconds_for_end_time * self._active_video_fps), self._active_video_total_frames - 1)
            ...
            QMessageBox.critical(self, "Input Error",
                                 f"Could not parse end condition: {e}")
            return

        new_segment_obj = TrackingSegment(
            video_path=self._active_video_path, fps=self._active_video_fps,
        ...
        if self._editing_segment_obj is not None and self._editing_segment_row is not None:
            # Update existing segment in the model
            new_segment_obj.unique_id = self._editing_segment_obj.unique_id  # Preserve ID on update
    ...
    def _on_table_selection_changed(self):
        ...
        if not can_edit_delete and self._editing_segment_obj is not None:  # Selection cleared while in edit mode
    ...
    def _on_edit_selected_segment_button_clicked(self):
        ...
        if segment_to_edit:
            ...
            self._on_annotation_frame_input_changed(
                segment_to_edit.annotated_frame)  # Validate
            ...
            self.duration_spinbox.setValue(
                max(1, int(segment_to_edit.duration_sec)))  # Ensure at least 1 sec
            self._toggle_end_condition_inputs_enabled_state()
    ...
    def _on_delete_selected_segment_button_clicked(self):
        ...
        segment_to_delete = self._segment_model.get_segment_at_row(
            row_to_delete)
        ...
        if reply == QMessageBox.Yes:
            ...
            if self._editing_segment_row == row_to_delete:  # If the deleted was being edited
                self._reset_form_to_defaults()
    ...
    def _on_track_all_defined_segments_button_clicked(self):
        ...
        if invalid_count > 0:
            ...
            reply = QMessageBox.warning(self, "Invalid Segments Found", msg,
                                        QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.No:
                return
        ...
        if not valid_segments_to_track:
            QMessageBox.warning(
                self, "No Valid Segments", "There are no valid segments (with annotations) to track.")
            return
    ...
    def get_defined_segments(self) -> List[TrackingSegment]:
        """Called by AnnolidWindow if 'OK' is pressed to retrieve all (valid or not) segments."""
        return self._segment_model.get_all_segments()

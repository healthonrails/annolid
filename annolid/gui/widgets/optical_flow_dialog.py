...
class FlowOptionsDialog(QtWidgets.QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        backend_val = (
            str(default_backend).lower() if default_backend is not None else "farneback"
        )
        ...
        raft_val = (
            str(default_raft_model).lower()
            if default_raft_model is not None
            else "small"
        )
        ...
        viz_val = str(default_viz).lower() if default_viz is not None else "quiver"
        ...
        self.opacity_value_label = QtWidgets.QLabel(f"{self.opacity_slider.value()}%")
        ...
        farneback_form.addRow(QtWidgets.QLabel("Pyramid scale:"), self.pyr_scale_spin)
        farneback_form.addRow(QtWidgets.QLabel("Levels:"), self.levels_spin)
        farneback_form.addRow(QtWidgets.QLabel("Window size:"), self.winsize_spin)
        farneback_form.addRow(QtWidgets.QLabel("Iterations:"), self.iterations_spin)
        farneback_form.addRow(QtWidgets.QLabel("Poly N:"), self.poly_n_spin)
        farneback_form.addRow(QtWidgets.QLabel("Poly Sigma:"), self.poly_sigma_spin)
        ...
        quiver_form.addRow(QtWidgets.QLabel("Step (density):"), self.quiver_step_spin)
        quiver_form.addRow(
            QtWidgets.QLabel("Gain (arrow length):"), self.quiver_gain_spin
        )
        ...
        self.backend_combo.currentIndexChanged.connect(self._update_enabled_states)
    ...
    def values(
        ...
        Tuple[str, str, str, int, int, float, bool, float, int, int, int, int, float]
    ]:

...
from annolid.segmentation.SAM.efficienttam.utils.misc import mask_to_box
...
def sample_box_points(
    ...
    if noise > 0.0:
        ...
        box_noise = box_noise * \
            torch.stack((max_dx, max_dy, max_dx, max_dy), dim=-1)

        box_coords = box_coords + box_noise
        img_bounds = (
            torch.tensor([W, H, W, H], device=device) - 1
        )  # uncentered pixel coords
        box_coords.clamp_(torch.zeros_like(img_bounds),
                          img_bounds)  # In place clamping

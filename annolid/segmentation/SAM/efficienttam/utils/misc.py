...
def get_connected_components(mask):
    ...
    """
    if _C is None:
        # Fallback: treat all foreground as a single component with zero area,
        # effectively disabling hole-filling while keeping the API intact.
        labels = torch.zeros_like(mask, dtype=torch.int64)
...
def load_video_frames_from_jpg_images(
    ...
    The upstream EfficientTAM implementation expects JPEG files named
    "<frame_index>.jpg".  In Annolid, extracted frames may be stored as
    either JPEG or PNG.  Here we support both extensions and fall back
    to lexicographic ordering when filenames are not pure integers.
    """
    if isinstance(video_path, str) and os.path.isdir(video_path):
        jpg_folder = video_path
    else:
        raise NotImplementedError(
            "Only image-frame folders are supported at this moment. For video files, "
            "you may use ffmpeg (https://ffmpeg.org/) to extract frames into a folder "
            "of JPEG or PNG files, such as:\n"
            "  ffmpeg -i <your_video>.mp4 -q:v 2 -start_number 0 <output_dir>/'%05d.jpg'\n"
        )

    exts = {".jpg", ".jpeg", ".png", ".JPG", ".JPEG", ".PNG"}
    frame_names = [
        p for p in os.listdir(jpg_folder) if os.path.splitext(p)[-1] in exts
    ]
    # Prefer numeric ordering when possible, but fall back to lexicographic.
    try:
...
def load_video_frames_from_video_file(
    ...
    We first try to use ``decord`` (as in the upstream EfficientTAM/SAM2
    ...
    try:  # pragma: no cover - optional dependency
        ...
        while True:
            ...
            frame_resized = cv2.resize(
                frame_rgb, (image_size,
                            image_size), interpolation=cv2.INTER_LINEAR
            )
            tensor = torch.from_numpy(frame_resized).permute(
                2, 0, 1).float() / 255.0
        ...
        first = reader.next()
        video_height, video_width, _ = first.shape
        images = [first.permute(2, 0, 1)]
        for frame in reader:

        ...
        if boxRPB != "none":
            ...
            if resolution is not None and stride is not None:
                ...
                coords_h, coords_w = self._get_coords(
                    feat_size, feat_size, device=select_device(None)
                )
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if torch.compiler.is_dynamo_compiling() or self.compilable_stored_size == (
            ...
            if feat_size not in self.coord_cache:
                self.coord_cache[feat_size] = self._get_coords(H, W, target_device)
        ...
        if coords_h.device != target_device:
            coords_h = coords_h.to(device=target_device)
        if coords_w.device != target_device:
            coords_w = coords_w.to(device=target_device)

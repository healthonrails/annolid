...
def edt_triton(data: torch.Tensor):
    ...
    If Triton is available, runs the GPU kernel. Otherwise falls back to a
    CPU-based SciPy implementation (if installed) to avoid hard import failures.
    ...
    if _HAS_TRITON:
        if not data.is_cuda:
            raise ValueError("edt_triton expects CUDA tensors when Triton is available.")
        assert data.dim() == 3
    ...
    # CPU fallback using SciPy if Triton is unavailable
    try:
        import scipy.ndimage as ndi  # type: ignore
    except ModuleNotFoundError as exc:
        raise ImportError(
            "edt_triton fallback requires SciPy when Triton is not installed."
        ) from exc

    data_cpu = data.detach().to("cpu")
    if data_cpu.dtype != torch.bool:
        data_cpu = data_cpu.bool()
    # SciPy EDT computes distance to the nearest zero; invert to match Triton behavior.
    dist = np.stack(
        [ndi.distance_transform_edt(~data_cpu[i].numpy()) for i in range(data_cpu.shape[0])]
    )
    dist_t = torch.from_numpy(dist).to(data.device, dtype=torch.float32)
    return dist_t

...
import annolid.segmentation.maskrcnn.transforms as T
...
class FilterAndRemapCocoCategories(object):
    ...
    def __call__(self, image, target):
        ...
        for obj in anno:
            obj["category_id"] = self.categories.index(
                obj["category_id"]
            )
        target["annotations"] = anno
        return image, target
...
class ConvertCocoPolysToMask(object):
    def __call__(self, image, target):
        ...
        segmentations = [
            obj['segmentation'] for obj in anno
        ]
...
def _coco_remove_images_without_annotations(
    ...
    def _has_only_empty_bbox(anno):
        return all(any(o <= 1 for o in obj["bbox"][2:])
                   for obj in anno
                   )

    def _count_visible_keypoints(anno):
        return sum(sum(
            1 for v in ann["keypoints"][2::3]
            if v > 0
        ) for ann in anno
        )
...
def get_coco(root, image_set, transforms, mode='instances'):
    anno_file_name = "annotations.json"
    ...
    root_dataset_path = Path(root)
    img_folder = root_dataset_path / image_set
    ann_file = root_dataset_path / image_set / anno_file_name

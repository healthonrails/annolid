...
from annolid.segmentation.cutie_vos.model.group_modules import *
from annolid.segmentation.cutie_vos.model.utils import resnet
...
class MaskEncoder(nn.Module):
    ...
    def forward(self,
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            g_chunk = g_chunk.view(
                batch_size, actual_chunk_size, *g_chunk.shape[1:])
...
class PixelFeatureFuser(nn.Module):
    def __init__(self, model_cfg: DictConfig, single_object=False):
        ...
        if self.single_object:
            self.sensory_compress = GConv2d(
                sensory_dim + 1, value_dim, kernel_size=1)
        else:
            self.sensory_compress = GConv2d(
                sensory_dim + 2, value_dim, kernel_size=1)
    ...
    def forward(self,
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            with torch.amp.autocast('cuda',enabled=False):
                logits = self.pred(
                    F.relu(p4.flatten(start_dim=0, end_dim=1).float()))

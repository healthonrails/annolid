...
class SegmentedCutieExecutor:
    ...
    def __init__(
        ...
        self._optical_flow_settings = optical_flow_settings_from(processing_config)
    ...
    def _initialize_engine(self):
        ...
        logger.info(f"segment {self.segment_start_frame}-{self.segment_end_frame}")
        try:
            ...
            logger.error(
                f"Failed to initialize CutieEngine for {self.video_path.name}: {e}",
                exc_info=True,
            )
    ...
    def _prepare_initial_mask(self) -> bool:
        ...
        if not annotation_json_path.exists():
            ...
            logger.error(f"for segment(annotated at {self.annotated_frame}) not found.")
            return False

        try:
            label_file = LabelFile(str(annotation_json_path), is_video_frame=True)
            ...
            annotated_frame_image = temp_video_loader.load_frame(self.annotated_frame)
            ...
            self.initial_mask_info = {
                ...
                # {1: 'animal_1', ...}
                "labels_map_id_to_name": {
                    v: k for k, v in labels_dict_numeric_ids.items()
                },
            ...
            logger.error(
                f"Error preparing initial mask from {annotation_json_path.name}: {e}",
                exc_info=True,
            )
            return False
    ...
    def _initialize_segment_video_writer(self, fps: float):
        ...
        if (
            self.config.get("save_video_with_color_mask", False)
            and self.initial_mask_info
        ):
            output_video_path_str = str(
                self.video_folder
                / f"{self.video_path.stem}_segment_{self.segment_start_frame}_to_{self.segment_end_frame}_overlay.mp4"
            )
            ...
            logger.info(f"Recording segment overlay to: {output_video_path_str}")

    # To calculate motion index and store per-object data for the current frame
    def _process_object_metrics(
        self,
        object_label_str: str,
        object_mask_np: np.ndarray,  # Boolean mask for this single object
        dense_flow_xy: Optional[np.ndarray],
    ):  # Optical flow (dx, dy) for the current frame transition
        ...
        if dense_flow_xy is not None and self.config.get("compute_optical_flow", True):
            magnitude = np.sqrt(dense_flow_xy[..., 0] ** 2 + dense_flow_xy[..., 1] ** 2)
        ...
        if (
            self._current_frame_idx_for_saving is not None
        ):  # Ensure frame context is set
    ...
    def _save_frame_annotation(
        self,
        frame_idx: int,
        predicted_mask_np_object_ids: np.ndarray,  # Mask with numeric object IDs
        labels_map_id_to_name: Dict[int, str],
        frame_shape: tuple,
        dense_flow_for_this_frame: Optional[np.ndarray],
    ):  # Pass the computed flow
        """Saves the predicted mask for a single frame as a JSON annotation, including motion index."""
        self._current_frame_idx_for_saving = (
            frame_idx  # Set for _process_object_metrics
        )

        filename_json = (
            self.video_folder / f"{self.video_path.stem}_{frame_idx:09d}.json"
        )
    ...
    def process_segment(self) -> str:
        ...
        frames_to_propagate_this_segment = (
            self.segment_end_frame - self.segment_start_frame + 1
        )
        ...
        self._initialize_segment_video_writer(video_fps)  # Init writer if recording
        ...
        if (
            self.config.get("compute_optical_flow", True)
            and self.segment_start_frame > 0
        ):
        ...
        try:
            ...
            for (
                frame_idx,
                current_frame_bgr,
                pred_mask_np_obj_ids,
            ) in self.cutie_engine.process_frames(
                video_capture=cap,
                ...
                if (
                    self.config.get("compute_optical_flow", True)
                    and prev_frame_bgr is not None
                ):
                ...
                if self.video_writer:
                    overlay = overlay_davis(current_frame_bgr, pred_mask_np_obj_ids)
                    if dense_flow_for_this_frame is not None and self.config.get(
                        "compute_optical_flow", False
                    ):
                        mask_for_flow_viz = pred_mask_np_obj_ids > 0
                        overlay_with_flow = draw.draw_flow(
                            overlay.copy(),
                            dense_flow_for_this_frame
                            * np.expand_dims(mask_for_flow_viz, axis=-1),
                        )
            ...
            final_message = (
                f"Segment processing completed. Processed {frames_actually_processed_count} frames "
                f"in range [{self.segment_start_frame}-{self.segment_end_frame}]."
            )
        except Exception as e:
            logger.error(
                f"Exception during segment processing for {self.video_path.name}: {e}",
                exc_info=True,
            )
            ...
            if self._csv_frame_numbers:  # If any data was collected
                ...
                try:
                    ...
                    logger.info(f"Saved tracking CSV for segment to: {csv_filename}")
                except Exception as e:
                    logger.error(
                        f"Failed to save tracking CSV for segment {csv_filename.name}: {e}",
                        exc_info=True,
                    )
    ...
    def cleanup(self):
        ...
        logger.debug(f"SegmentedCutieExecutor: Cleaning up for {self.video_path.name}")
        if self.video_writer:
            self.video_writer.release()
            self.video_writer = None

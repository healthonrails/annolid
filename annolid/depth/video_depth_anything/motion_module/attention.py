...
def warn_xformers_unavailable():
    global _XFORMERS_WARNING_SHOWN
    if not _XFORMERS_WARNING_SHOWN:
        logger.warning(
            "xFormers not available, falling back to PyTorch attention.")
...
class CrossAttention(nn.Module):
    ...
    def reshape_heads_to_batch_dim(self, tensor):
        ...
        tensor = tensor.reshape(batch_size, seq_len,
                                head_size, dim // head_size).contiguous()
        tensor = tensor.permute(0, 2, 1, 3).reshape(
            batch_size * head_size, seq_len, dim // head_size).contiguous()
        return tensor
    ...
    def reshape_batch_dim_to_heads(self, tensor):
        ...
        tensor = tensor.reshape(batch_size // head_size,
                                head_size, seq_len, dim).contiguous()
        tensor = tensor.permute(0, 2, 1, 3).reshape(
            batch_size // head_size, seq_len, dim * head_size).contiguous()
        return tensor
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        use_xformers_attention = XFORMERS_AVAILABLE and self._use_memory_efficient_attention_xformers
        if self._use_memory_efficient_attention_xformers and not XFORMERS_AVAILABLE:
            warn_xformers_unavailable()
...
def apply_rotary_emb(
    ...
    xq_ = torch.view_as_complex(xq.float().reshape(
        *xq.shape[:-1], -1, 2).contiguous())
    xk_ = torch.view_as_complex(xk.float().reshape(
        *xk.shape[:-1], -1, 2).contiguous())

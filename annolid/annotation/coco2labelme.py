...
class COCO2Labeme:
    ...
    folder that contains the images as provided in annotation images dict.
    Usage:
    ...
    def __init__(self, annotations, images_dir) -> None:
        ...
        for i, img in enumerate(images):
            ...
            for annos in annotations:
                if int(annos["image_id"]) == int(img_id):
                    ...
                    s = Shape(label=class_names[cat_id], shape_type="polygon", flags={})
                    for k in range(0, len(points) - 1, 2):
                        s.addPoint((points[k], points[k + 1]))
                    label_list.append(s)
            save_labels(img_file_json, img_file_name, label_list, img_height, img_width)
...
if __name__ == "__main__":
    cl = COCO2Labeme("/path/to/dataset_coco/", "/path/to/dataset_coco/")
    cl.convert()

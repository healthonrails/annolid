...
from annolid.core.media.video import (
    get_keyframe_timestamps as _get_keyframe_timestamps,
    get_video_fps as _get_video_fps,
)

get_keyframe_timestamps = _get_keyframe_timestamps
get_video_fps = _get_video_fps
...
def extract_subclip(video_file, start_time, end_time):
    video_dir = Path(video_file).parent
    out_video_name = Path(video_file).stem + f"_{str(start_time)}_{str(end_time)}.mp4"
    out_video_path = video_dir / out_video_name

    ffmpeg_extract_subclip(
        video_file, start_time, end_time, targetname=str(out_video_path)
    )
    ...
    for mode, value in extraction_points:
        ...
        if frame_idx < 0:
            frame_idx = int(
                value
                if mode == "index"
                else round(value * (cap.get(cv2.CAP_PROP_FPS) or 0))
            )
        ...
        out_frame_file = (
            out_dir / f"{(video_name).replace(' ', '_')}_{frame_idx:08}.png"
        )
...
def download_youtube_video(
    ...
    *,
    ...
    destination_root = (
        Path(output_dir) if output_dir else Path.home() / "annolid_youtube"
    )
...
from annolid.core.media.video import CV2Video  # noqa: E402
...
def extract_frames(
    ...
    while ret:
        ...
        if num_frames == -1:
            ...
            if ret:
                ...
                logger.info("Saved frame %s to %s", frame_number, out_frame_file)
            continue
        ...
        if algo == "flow" and num_frames != -1:
            ...
            try:
                ...
                progress_msg = f"processing frame {frame_number}, the diff to prev frame is {q_score}."

                if len(keeped_frames) < num_frames:
                    heapq.heappush(keeped_frames, ((q_score, frame_number, frame)))
                else:
                    heapq.heappushpop(keeped_frames, ((q_score, frame_number, frame)))
    ...
    for kf in keeped_frames:
        ...
        if prediction and num_frames <= 5:
            try:
                from annolid.segmentation.maskrcnn import (
                    inference as _maskrcnn_inference,
                )

                _maskrcnn_inference.predict_mask_to_labelme(out_img, 0.7)
            except Exception as exc:
                logger.error(
...
def track(video_file=None, name=None, weights=None):

...
def extract_polygons_from_frame(
    json_path: Path, num_points: int
...
def process_sequence(
    sequence_path: Path, num_points: int
    ...
    for json_file in sorted(sequence_path.glob("*.json")):
        ...
        if keypoints is None:
            logger.warning("Skipping frame %s due to extraction error.", json_file)
            continue
        frames_keypoints.append(keypoints)
    return {"keypoints": frames_keypoints}


def build_group_sequences(
    group_folder: Path, num_points: int
    ...
    for seq_path in sorted([p for p in group_folder.iterdir() if p.is_dir()]):
        logger.info("Processing sequence '%s' in '%s'", seq_path.name, group_folder)
        sequences[seq_path.name] = process_sequence(seq_path, num_points)
    return sequences


def save_group_dataset(
    group_label: str, sequences: Dict[str, Dict[str, object]], output_folder: Path
) -> None:
    ...
    for seq, sequence_data in sequences.items():
        ...
        np.save(output_path, {group_label: {seq: sequence_data}}, allow_pickle=True)
        logger.info(
            "Saved sequence '%s' (group: '%s') to '%s'", seq, group_label, output_path
        )
...
def main() -> None:
    ...
    train_sequences = build_group_sequences(Path(args.train_folder), args.num_points)
    test_sequences = build_group_sequences(Path(args.test_folder), args.num_points)
